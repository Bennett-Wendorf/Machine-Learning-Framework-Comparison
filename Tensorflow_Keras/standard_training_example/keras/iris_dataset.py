...
class IrisDataset():
    ...
    def get_from_train_test_splits(self, train_percent=DEFAULT_TRAIN_PERCENT):
        ...
        return (
                tf.convert_to_tensor(train_dataset[["SepalLength", "SepalWidth", "PetalLength", "PetalWidth"]].to_numpy(), np.float32), 
                tf.convert_to_tensor(train_dataset["Name"].to_numpy(), np.float32)
            ), (
                tf.convert_to_tensor(test_dataset[["SepalLength", "SepalWidth", "PetalLength", "PetalWidth"]].to_numpy(), np.float32), 
                tf.convert_to_tensor(test_dataset["Name"].to_numpy(), np.float32)
            )
